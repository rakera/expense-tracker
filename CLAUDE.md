...
Рабочая (интеграционная) ветка — **`dev`**. Под каждую задачу оттягивай от `dev` отдельную ветку `feat_<название>` (название одним словом), работай и коммить в неё, затем вливай обратно в `dev` (merge / PR в `dev`).

**Ветку `main` не трогаем:** не переключаться на неё, не коммитить, не мержить в неё, не пушить в неё и не открывать в неё pull request. `main` обновляется только вручную владельцем репозитория; никакие git-операции не должны быть направлены на `main`.
...
**Pull request**: открывай PR из `feat_<название>` **в `dev`** — перед созданием смотри `git diff dev`, чтобы написать информативное описание (что реализовано, какие endpoints добавлены). Title — по Conventional Commits. PR в `main` из-под агента не создаём — их открывает владелец репозитория вручную.
